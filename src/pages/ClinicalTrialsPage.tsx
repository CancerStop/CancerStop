...
const dataGridColumns: GridColDef[] = [
  ...
  },
  {
    ...
    headerName: "Type",
    description:
      "Interventional studies study the effect of an intervention, such as administering a new medication. Observational studies measure effects without attempting to change the outcome (no treatments are given).",
    minWidth: 130,
    flex: 0.1,
  },
  {
  ...
  },
  {
    ...
    renderCell(params) {
      if (typeof params.value === "boolean") {
        return params.value ? (
          <a
            href={`https://www.clinicaltrials.gov/ct2/show/results/NCT02336282`}
            target="_blank"
            rel="noreferrer"
          >
            Yes
  ...
  },
  {
    ...
    renderCell(params) {
      if (Array.isArray(params.value)) {
        ...
        if (text.length > 200)
          return <span title={text}>{text.slice(0, 197) + "..."}</span>;
        else return text;
      } else return `TypeError: ${typeof params.value}`;
    },
    minWidth: 200,
  ...
  },
  {
    ...
    headerName: "Phase",
    description:
      "A Phase 1 study is performed with 20 to 80 participants to determine safe dosage. Phase 2 studies involve hundreds of participants, and check for long term side effects. Phase 3 and 4 studies involve thousands of participants.",
    renderCell(params) {
      if (Array.isArray(params.value)) {
        if (
          params.value.length &&
          params.value.every((p) => /Phase \d/.test(p))
        )
          return `Phase ${params.value.map((p) => p.split(" ")[1]).join(", ")}`;
        else if (
          params.value.length === 0 ||
          params.value[0] === "Not Applicable"
        ) {
          return "N/A";
        }
...
export default function ClinicalTrialsPage() {
  ...
  const [searchStatus, setSearchStatus] = useState<
    "idle" | "searching" | "errored"
  >("idle");

  const fetchData = (
    page = paginationModel.page,
    resultsPerPage = paginationModel.pageSize
  ) =>
    findStudies(
  ...
  const search = (model = paginationModel) => {
    ...
    fetchData(model.page, model.pageSize)
      .then((data) => {
        if (
          model.page > Math.ceil(data.totalStudiesAvailable / model.pageSize) &&
  ...
  return (
    <div className="cancerSpecificClinicalTrialsTemplate clinicalTrialsPage">
      <SubHeader>
        <a
          href="https://clinicaltrials.gov/ct2/"
          target="_blank"
          rel="noreferrer"
        >
          Clinical Trials
      ...
      {!response && searchStatus === "searching" ? (
        <div className="table-text">Loading...</div>
      ) : !response ? (
        ""
      ) : response.totalStudiesAvailable === 0 ? (
        <div className="table-text">No studies found.</div>
      ) : (
        <DataGrid
